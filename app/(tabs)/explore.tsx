...
function computeSIP(monthly: number, annualRate: number, years: number): number {
...
function computeLumpSum(principal: number, annualRate: number, years: number): number {
  ...
  return (
    <View style={cs.card}>
      <View style={cs.titleRow}>
        <Ionicons name="calculator" size={20} color="#FF8C00" />
        <Text style={cs.title}>Investment Calculator</Text>
      </View>

      {/* Mode toggle */}
      <View style={cs.toggle}>
        <TouchableOpacity style={[cs.toggleBtn, mode === 'sip' && cs.toggleOn]} onPress={() => setMode('sip')}>
          <Text style={[cs.toggleTxt, mode === 'sip' && cs.toggleTxtOn]}>Monthly SIP</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[cs.toggleBtn, mode === 'lumpsum' && cs.toggleOn]} onPress={() => setMode('lumpsum')}>
          <Text style={[cs.toggleTxt, mode === 'lumpsum' && cs.toggleTxtOn]}>Lump Sum</Text>
        </TouchableOpacity>
      </View>

      {/* Amount */}
      <Text style={cs.label}>{mode === 'sip' ? 'Monthly SIP Amount' : 'Lump Sum Amount'}</Text>
      ...
      >
        <View style={[cs.sIcon, { backgroundColor: fund.color + '22' }]}>
          <Ionicons name={fund.icon as React.ComponentProps<typeof Ionicons>['name']} size={16} color={fund.color} />
        </View>
        <View style={{ flex: 1 }}>
          <Text style={[cs.sName, { color: fund.color }]}>{fund.name}</Text>
          <Text style={cs.sRate}>~{fund.avgRate}% avg returns · {fund.riskLabel} risk</Text>
        </View>
        <Ionicons name={showPicker ? 'chevron-up' : 'chevron-down'} size={16} color="rgba(255,255,255,0.35)" />
      </TouchableOpacity>
      ...
      {showPicker && (
        <View style={cs.dropdown}>
          {FUNDS.map((f) => (
            <TouchableOpacity
              key={f.id}
              style={[cs.ddItem, f.id === selectedId && { backgroundColor: f.color + '18' }]}
              onPress={() => { setSelectedId(f.id); setShowPicker(false); }}
              activeOpacity={0.8}
            >
      ...
      {/* Result */}
      {hasResult ? (
        <View style={[cs.result, { borderColor: fund.color + '55', backgroundColor: fund.color + '0F' }]}>
          <Text style={cs.resultSub}>
            {mode === 'sip' ? `₹${amtNum.toLocaleString('en-IN')}/mo` : `₹${amtNum.toLocaleString('en-IN')} lumpsum`}{' '}
            in {fund.name} for {yrsNum}y
          </Text>
          <Text style={[cs.resultAmt, { color: fund.color }]}>{formatAmount(fv)}</Text>
          <View style={cs.resultRow}>
            ...
            <View style={cs.rcell}>
              ...
              <Text style={[cs.rval, { color: '#4ADE80' }]}>{formatAmount(gains)}</Text>
            </View>
            <View style={cs.rdiv} />
            <View style={cs.rcell}>
              <Text style={cs.rlabel}>Returns</Text>
              <Text style={[cs.rval, { color: '#4ADE80' }]}>
                {invested > 0 ? `${((gains / invested) * 100).toFixed(0)}%` : '—'}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={cs.empty}>
          <Text style={cs.emptyTxt}>Fill in the fields above to see projected wealth</Text>
        </View>
      )}
    </View>
  );
    ...
    <View style={[fc.card, { borderColor: fund.color + '44' }]}>
      <View style={fc.row}>
        <View style={[fc.icon, { backgroundColor: fund.color + '22' }]}>
          <Ionicons name={fund.icon as React.ComponentProps<typeof Ionicons>['name']} size={20} color={fund.color} />
        </View>
        <View style={{ flex: 1 }}>
        ...
        <View style={fc.badge}>
          <Text style={[fc.badgeTxt, { color: fund.color }]}>~{fund.avgRate}%</Text>
        </View>
      </View>
      <View style={fc.meta}>
        <View style={fc.chip}>
      ...
      >
        ...
        >
          <View style={{ marginBottom: 20, marginTop: 8 }}>
            <Text style={{ fontSize: 28, fontWeight: '800', color: '#FFF', fontFamily: Fonts?.rounded ?? 'normal', marginBottom: 4 }}>
              Fund Explorer
            </Text>
          ...
          <Text style={{ fontSize: 16, fontWeight: '700', color: 'rgba(255,255,255,0.6)', marginBottom: 12 }}>
            All Instruments
          </Text>
          ...
          <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', textAlign: 'center', marginTop: 10, lineHeight: 16 }}>
            * Returns are historical averages. Mutual fund investments are subject to market risks.
          </Text>
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const cs = StyleSheet.create({
  card: { backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,140,0,0.35)', padding: 18, marginBottom: 24 },
  titleRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 16 },
  title: { fontSize: 17, fontWeight: '800', color: '#FFF' },
  toggle: { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 10, padding: 3, marginBottom: 16 },
  toggleBtn: { flex: 1, paddingVertical: 8, borderRadius: 8, alignItems: 'center' },
  toggleOn: { backgroundColor: '#FF8C00' },
  toggleTxt: { fontSize: 13, fontWeight: '600', color: 'rgba(255,255,255,0.45)' },
  toggleTxtOn: { color: '#FFF' },
  label: { fontSize: 10, fontWeight: '700', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 6 },
  inputBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', paddingHorizontal: 14, paddingVertical: 12 },
  prefix: { fontSize: 17, fontWeight: '700', color: '#FF8C00', marginRight: 4 },
  suffix: { fontSize: 14, color: 'rgba(255,255,255,0.4)', marginLeft: 6 },
  input: { flex: 1, fontSize: 17, fontWeight: '600', color: '#FFF', padding: 0 },
  selector: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 10, gap: 10 },
  sIcon: { width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  sName: { fontSize: 14, fontWeight: '700' },
  sRate: { fontSize: 11, color: 'rgba(255,255,255,0.38)', marginTop: 1 },
  dropdown: { marginTop: 6, backgroundColor: '#1A1A2E', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', overflow: 'hidden' },
  ddItem: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 11, gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: 'rgba(255,255,255,0.07)' },
  ddIcon: { width: 28, height: 28, borderRadius: 7, alignItems: 'center', justifyContent: 'center' },
  ddName: { flex: 1, fontSize: 13, color: '#FFF', fontWeight: '600' },
  ddRate: { fontSize: 12, fontWeight: '700' },
  result: { borderRadius: 14, borderWidth: 1, padding: 16, marginTop: 14 },
  resultSub: { fontSize: 11, color: 'rgba(255,255,255,0.42)', marginBottom: 4 },
  resultAmt: { fontSize: 34, fontWeight: '800', marginBottom: 16 },
  resultRow: { flexDirection: 'row' },
  rcell: { flex: 1, alignItems: 'center' },
  rlabel: { fontSize: 10, color: 'rgba(255,255,255,0.38)', marginBottom: 3 },
  rval: { fontSize: 14, fontWeight: '700', color: '#FFF' },
  rdiv: { width: 1, backgroundColor: 'rgba(255,255,255,0.1)', marginHorizontal: 6 },
  empty: { padding: 18, alignItems: 'center', borderRadius: 12, borderWidth: 1, borderStyle: 'dashed', borderColor: 'rgba(255,255,255,0.12)', marginTop: 14 },
  emptyTxt: { fontSize: 13, color: 'rgba(255,255,255,0.28)', textAlign: 'center' },
});

const fc = StyleSheet.create({
  card: { backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 16, borderWidth: 1, padding: 14, marginBottom: 12 },
  ...
  tagline: { fontSize: 11, color: 'rgba(255,255,255,0.42)' },
  badge: { backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4 },
  badgeTxt: { fontSize: 12, fontWeight: '700' },
  meta: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 10, flexWrap: 'wrap' },
  chip: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 5, paddingHorizontal: 7, paddingVertical: 3 },
  chipTxt: { fontSize: 10, color: 'rgba(255,255,255,0.42)' },
  desc: { fontSize: 12, color: 'rgba(255,255,255,0.42)', marginTop: 9, lineHeight: 18 },
});
